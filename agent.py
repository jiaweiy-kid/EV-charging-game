...
# Q_net = QNetwork(torch.randn(1, 53).shape[1], torch.tensor([0.]).shape[0], args.hidden_size)
# Q_target = QNetwork(torch.randn(1, 53).shape[1], torch.tensor([0.]).shape[0], args.hidden_size)
class Agent:
    def __init__(self, ev_id, global_env, writer, bus=-1, episode_times=1000, d1_distribution=None,
    ...
    def online_interact(self, t_tongb):
        episode_states = []
        episode_times = []

        done = False
        # random.seed()
        state = self.env.reset(t_tongb)
        self.episode_reward = np.array([0.0], dtype='f8')
        self.anx_reward = np.array([0.0], dtype='f8')
        self.price_reward = np.array([0.0], dtype='f8')
        self.power_reward = np.array([0.0], dtype='f8')
        ...
        while not done:
            if args.start_steps > self.total_numsteps:
                action = np.array([np.random.uniform(-0.2, 1)], dtype='f8')
            else:
                action = self.agent.select_action(state)

            episode_times.append(self.env.t)
            ...
            # episode_states.append([state, action, reward, next_state, 1.0])
            episode_states.append([state, action, reward, next_state, float(not done)])

            state = next_state
            self.total_numsteps += 1
        return episode_states, episode_times
        ...
        for i in range(len(episode_states)):
            ...
            for j in range(len(clients_episode_actions)):  # 遍历所有agent的episode action
                if i < len(clients_episode_actions[j]):  # 如果agent在这一步有action，则放入，否则放入0.0
                    ...
                    other_state_next[j] = clients_episode_states[j][-1][3][-5:].tolist()
            # print(episode_state[0][0:48].shape)
            # print(other_state)
            # print(np.array(sum(other_state, [])).shape)
...
# class Leader:
#     def __init__(self, global_env, writer, num_agent):
